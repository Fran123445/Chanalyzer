...
@router.get("/similar/semantic/simple")
def get_similar_to_text_list(text_list: list[str], weights_list: list[int], request: Request, top_n = 10):
    return request.app.state.board_finder.find_similar_to_text_list(text_list, weights_list, top_n)

@router.get("/similar/semantic/multiple")
def get_similar_to_multiple_text_lists(text_lists: list[list[str]], weight_lists: list[list[int]], request: Request, top_n = 10):
